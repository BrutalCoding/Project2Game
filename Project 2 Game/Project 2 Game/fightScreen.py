...
class fightScreen(object):
    ...
    def __init__(self,fighterCurrentPlayerCounter,screen,currentPlayerCounter,
        ...
        self.setDefaultSoundSystem = setDefaultSoundSystem
        self.enableSound = enableSound
        self.gameStatus = gameStatus

    def startFight(self):
        for ev in self.events:
            if self.events != [] and ev.type == pygame.QUIT:
                gameIsRunning = False
                print("Fightscreen ESC werkt")
        if self.fighterCurrentPlayerCounter == 2:
            if self.events != [] and ev.type == pygame.KEYDOWN:
                if ev.key == pygame.K_SPACE:
                    self.setDefaultSoundSystem(self.enableSound,"Sounds\Intro_1_Soft_Pump.mp3", 300, 0.3)
        ...
        curplaypos = self.selectedCharacters[self.tempCurrentPlayerCounter].Tile #currentPlayerCounter got updated to the next player, but we want the
                                                                                 #prself.evious player.
        ...
        #If the first turn has not begun yet, display a placeholder for the
        #dice.  Else show what dice was thrown.
        if self.fighterCurrentPlayerCounter == 0:
            ...
            self.screen.blit(self.dice[self.fighterDieInt], (((self.screen.get_width() / 2) - 95), (self.screen.get_height() / 2) - 95))
        ...
        if fightDie.collidepoint(pygame.mouse.get_pos()) and self.fighterCurrentPlayerCounter < 2: #If there are still turns left and
            print(self)
            if self.events != [] and ev.type == pygame.MOUSEBUTTONDOWN:
                    self.fighterDieInt = random.randint(1,6)
